	...
	for (let x = 0; x < props.code.length; x++) {
		do {
			...
			if (acceptedChars.includes(char)) {
				...
				else if (char === '>') { pointer++; if (pointer === memory.length) { if (props.maxMem === 'auto' || memory.length < props.maxMem) { memory.push(0) } else { throw `Error: memory cannot pass '${props.maxMem}' arrays.` } } }
